...
toggleCommercialLayer(layer: any) {
  ...
  switch (layer.id) {
    ...
    case "sec_lectura":
      this.secuenciaLecturaLayer.setVisible(layer.active);
      break;
...
seleccionarSector(codigo: string): void {
  ...
  source: new TileWMS({
    ...
    this.map = new OlMap({
      target: "map",
      layers: [base, this.lotesLayer, this.lecturasLayer, this.secuenciaLecturaLayer, this.sectoresComercialesLayer, this.callesLayer],
      view: new View({
